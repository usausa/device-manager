...
public static class DataSeeder
{
    ...
    private static void SeedDeviceStatuses(SqliteConnection connection)
    {
        ...
        var statuses = new[]
        {
            ("dev-001",         0, 85.0,  80.0, 90.0, 72,         (int?)-52, 42.0, 61.0, 35.6812,  139.7671,  "{\"scanCount\":1423}"),
            ("dev-002",         0, 62.5,  55.0, 70.0, 58,         (int?)-68, 38.0, 57.0, 35.6815,  139.7675,  "{\"scanCount\":987}"),
            ("tab-a",           0, 100.0, 100.0, 96.0, 91,        (int?)-45, 22.0, 48.0, 34.0522, -118.2437,  "{\"activeSession\":true}"),
            ("tab-b",           1, 0.0,   5.0,  0.0,  34,         (int?)-78, 76.0, 82.0, 34.0525, -118.2440,  "{\"activeSession\":false}"),
            ("kiosk-tky-01",    0, 50.0,  48.0, 53.0, (int?)null, (int?)-55, 31.0, 44.0, 35.6895,  139.6917,  "{\"displayOn\":true,\"uptimeHours\":148}"),
            ("kiosk-tky-02",    2, 48.0,  46.0, 51.0, (int?)null, (int?)-82, 63.0, 71.0, 35.6897,  139.6920,  "{\"displayOn\":true,\"tempCelsius\":42}"),
            ("sensor-hq-01",    0, 100.0, 100.0, 100.0, 100,      (int?)-41, 18.0, 33.0, 37.7749, -122.4194,  "{\"humidity\":45,\"tempCelsius\":22}"),
            ("sensor-hq-02",    3, 0.0,   0.0,  0.0,  5,          (int?)null, 88.0, 92.0, 37.7750, -122.4195,  "{\"error\":\"SENSOR_FAULT\"}"),
            ("laptop-field-01", 0, 75.0,  70.0, 78.0, 63,         (int?)-60, 54.0, 68.0, 40.7128,  -74.0060,  "{\"vpnConnected\":true}"),
            ("rpi-lab-01",      2, 30.0,  28.0, 34.0, (int?)null, (int?)-75, 67.0, 73.0, 37.3861, -122.0839,  "{\"firmwareVersion\":\"0.9.3-beta\",\"crashCount\":7}"),
        };

        const string sql = """
            INSERT INTO DeviceStatus
                (DeviceId, Level, Progress, Progress1, Progress2, Battery, WifiRssi, CpuUsage, MemoryUsage, Latitude, Longitude, CustomData, Timestamp)
            VALUES
                (@DeviceId, @Level, @Progress, @Progress1, @Progress2, @Battery, @WifiRssi, @CpuUsage, @MemoryUsage, @Latitude, @Longitude, @CustomData, @Timestamp);
            """;

        foreach (var (deviceId, level, progress, progress1, progress2, battery, wifiRssi, cpuUsage, memoryUsage, lat, lon, customData) in statuses)
        {
            using var cmd = connection.CreateCommand();
            cmd.CommandText = sql;
            ...
            cmd.Parameters.AddWithValue("@Progress1", progress1);
            cmd.Parameters.AddWithValue("@Progress2", progress2);
