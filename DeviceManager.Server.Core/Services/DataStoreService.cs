...
public sealed class DataStoreService(DbConnectionFactory dbFactory, ILogger<DataStoreService> logger)
{
    ...
    public async Task SetCommonEntryAsync(string key, string value)
    {
        ...
        using var command = connection.CreateCommand();
        command.CommandText = """
            INSERT INTO CommonDataStore (Key, Value, CreatedAt, UpdatedAt) VALUES (@Key, @Value, @Now, @Now)
        ...
        logger.LogInformation("Common data store entry set: {Key}", key);
    }
    ...
    public async Task DeleteCommonEntryAsync(string key)
    {
        ...
        command.Parameters.AddWithValue("@Key", key);
        await command.ExecuteNonQueryAsync();
        logger.LogInformation("Common data store entry deleted: {Key}", key);
    }
    ...
    public async Task SetDeviceEntryAsync(string deviceId, string key, string value)
    {
        ...
        using var command = connection.CreateCommand();
        command.CommandText = """
            INSERT INTO DeviceDataStore (Key, Value, CreatedAt, UpdatedAt) VALUES (@Key, @Value, @Now, @Now)
            ON CONFLICT(Key) DO UPDATE SET Value = @Value, UpdatedAt = @Now
            """;
        command.Parameters.AddWithValue("@Key", key);
        command.Parameters.AddWithValue("@Value", value);
        command.Parameters.AddWithValue("@Now", now);
        await command.ExecuteNonQueryAsync();
        logger.LogInformation("Device data store entry set: {DeviceId}/{Key}", deviceId, key);
    }
    ...
    public async Task DeleteDeviceEntryAsync(string deviceId, string key)
    {
        ...
        command.Parameters.AddWithValue("@Key", key);
        await command.ExecuteNonQueryAsync();
        logger.LogInformation("Device data store entry deleted: {DeviceId}/{Key}", deviceId, key);
    }
