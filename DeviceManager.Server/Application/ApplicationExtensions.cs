...
using DeviceManager.Server.Application.Telemetry;
using DeviceManager.Server.Components;
using DeviceManager.Server.Endpoints;
...
using DeviceManager.Server.Infrastructure.ExceptionHandling;
using DeviceManager.Server.Infrastructure.HealthChecks;
using DeviceManager.Server.Infrastructure.Logging;
...
public static class ApplicationExtensions
{
    ...
    public static IHostApplicationBuilder ConfigureAuthentication(this IHostApplicationBuilder builder)
    {
        ...
        builder.Services
            ...
            .AddScheme<AuthenticationSchemeOptions, ApiKeyAuthenticationHandler>(ApiKeyDefaults.AuthenticationScheme, static _ => { });
        ...
        builder.Services.AddAuthorization(static options =>
        {
            ...
            // 端末(API キー)
            options.AddPolicy(Policies.Device, static policy => policy
                .AddAuthenticationSchemes(ApiKeyDefaults.AuthenticationScheme)
                .RequireAuthenticatedUser());
            // 端末または管理ユーザー
            options.AddPolicy(Policies.DeviceOrUser, static policy => policy
                .AddAuthenticationSchemes(ApiKeyDefaults.AuthenticationScheme, CookieAuthenticationDefaults.AuthenticationScheme)
                .RequireAuthenticatedUser());
        });
    ...
    public static IHostApplicationBuilder ConfigureOpenApi(this IHostApplicationBuilder builder)
    {
        builder.Services.AddOpenApi(static options =>
        {
            ...
            {
                document.Info.Title = "DeviceManager API";
                document.Info.Version = "v1";
                document.Info.Description = "Device management server API.";
                return Task.CompletedTask;
            });
        });
    ...
    public static IHostApplicationBuilder ConfigureComponents(this IHostApplicationBuilder builder)
    {
        ...
        builder.Services.AddDataAccessors(typeof(DeviceAccessor).Assembly);
        ...
        // Device (イベントバス / 接続追跡 / 送信)
        builder.Services.AddSingleton<DeviceEventBus>();
        builder.Services.AddSingleton<DeviceConnectionRegistry>();
        builder.Services.AddSingleton<DeviceMessenger>();

        // Worker
        builder.Services.AddHostedService<Workers.CleanupWorker>();
        ...
        builder.Services.AddOptions<DeviceSetting>().BindConfiguration("Device").ValidateDataAnnotations().ValidateOnStart();
        builder.Services.AddSingleton(static p => p.GetRequiredService<IOptions<DeviceSetting>>().Value);
        builder.Services.AddOptions<CleanupSetting>().BindConfiguration("Cleanup").ValidateDataAnnotations().ValidateOnStart();
        builder.Services.AddSingleton(static p => p.GetRequiredService<IOptions<CleanupSetting>>().Value);
    ...
    public static WebApplication MapEndpoints(this WebApplication app)
    {
        ...
        // Hub
        app.MapHub<DeviceHub>(DeviceHubConstants.Path);

        // gRPC (テレメトリ。HTTP/2 専用ポートで待受)
        app.MapGrpcService<Services.TelemetryGrpcService>();
    ...
    public static ValueTask InitializeApplicationAsync(this WebApplication app)
    {
        ...
        // Prepare database
        app.Services.GetRequiredService<DeviceService>().CreateTable();
        app.Services.GetRequiredService<LogService>().CreateTable();
        app.Services.GetRequiredService<ErrorReportService>().CreateTable();
        app.Services.GetRequiredService<MessageService>().CreateTable();
        app.Services.GetRequiredService<ConfigService>().CreateTable();
        app.Services.GetRequiredService<FunctionService>().CreateTable();
