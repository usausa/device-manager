...
public sealed class DeviceService
{
    ...
    public DeviceService(DbConnectionFactory dbFactory, ILogger<DeviceService> logger)
    {
    ...
    public async Task<List<DeviceSummary>> GetAllDevicesAsync()
    {
        ...
        command.CommandText = """
            SELECT d.DeviceId, d.Name, d."Group", d.Status, d.LastConnectedAt,
                   ds.Level, ds.Progress, ds.Battery, ds.Timestamp
            FROM Device d
        ...
        while (await reader.ReadAsync())
        {
            devices.Add(new DeviceSummary
            {
                ...
                StatusTimestamp = reader.IsDBNull(8) ? null : ParseDateTime(reader.GetString(8))
            });
        }
    ...
    public async Task<DeviceDetail?> GetDeviceAsync(string deviceId)
    {
        ...
        command.CommandText = """
            SELECT d.DeviceId, d.Name, d.Platform, d."Group", d.Tags, d.Note, d.Status,
                   ...
                   ds.Level, ds.Progress, ds.Battery, ds.Latitude, ds.Longitude,
        ...
        return new DeviceDetail
        {
            ...
            CustomData = reader.IsDBNull(15) ? null : JsonSerializer.Deserialize<Dictionary<string, string>>(reader.GetString(15)),
            StatusTimestamp = reader.IsDBNull(16) ? null : ParseDateTime(reader.GetString(16))
        };
    }
    ...
    public async Task RegisterDeviceAsync(DeviceRegistration registration)
    {
        ...
        // Ensure DeviceStatus row exists
        using var statusCmd = connection.CreateCommand();
        statusCmd.CommandText = """
            INSERT INTO DeviceStatus (DeviceId, Timestamp)
            VALUES (@DeviceId, @Timestamp)
        ...
        // Ensure device database exists
        _dbFactory.InitializeDeviceDatabase(registration.DeviceId);
    ...
    public async Task DeleteDeviceAsync(string deviceId)
    {
        ...
        _dbFactory.DeleteDeviceDatabase(deviceId);
    ...
    {
        // Update DeviceStatus in common DB
        using var connection = _dbFactory.GetCommonConnection();
        using var command = connection.CreateCommand();
        command.CommandText = """
        ...
        // Append to StatusHistory in device DB
