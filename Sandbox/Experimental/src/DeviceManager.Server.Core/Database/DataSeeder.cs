...
public static class DataSeeder
{
    public static void SeedCommonDatabase(DbConnectionFactory dbFactory, ILogger? logger = null)
    {
        ...
        logger?.LogInformation("Seeding common database with dummy data");
    ...
    private static void SeedDeviceStatuses(SqliteConnection connection)
    {
        ...
        // (DeviceId, Level, Progress, Battery, Latitude, Longitude, CustomData)
        var statuses = new[]
        {
    ...
    private static void SeedCommonConfig(SqliteConnection connection)
    {
        ...
        var configs = new[]
        {
            ...
            ("heartbeat.timeout", "120", "int", "Heartbeat timeout in seconds before marking device inactive"),
            ("notification.enabled", "true", "bool", "Enable push notifications for device alerts"),
            ("data.retention.days", "90", "int", "Number of days to retain historical data"),
        };
    ...
    private static void SeedCommonDataStore(SqliteConnection connection)
    {
        ...
        var entries = new[]
        {
            ...
            ("alert.rules", "[{\"metric\":\"battery\",\"op\":\"lt\",\"value\":20,\"severity\":\"warning\"},{\"metric\":\"battery\",\"op\":\"lt\",\"value\":10,\"severity\":\"error\"}]"),
            ("device.groups.order", "[\"Warehouse\",\"Sales\",\"Retail-JP\",\"HQ\",\"Field\",\"R&D\"]"),
            ("last.sync.summary", "{\"totalDevices\":10,\"activeDevices\":7,\"lastRun\":\"2026-03-28T00:00:00Z\"}"),
        };
