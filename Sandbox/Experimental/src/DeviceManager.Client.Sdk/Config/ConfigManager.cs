...
internal sealed class ConfigManager : IConfigManager, IDisposable
{
    ...
    {
        _logger.LogDebug("Config updated via gRPC: {Key}", entry.Key);
        _cache[entry.Key] = entry;
        SaveCacheToDisk();
    ...
    private void RegisterHubHandlers()
    {
        ...
        _subscriptions.Add(hub.On<ConfigEntry>(
            HubConstants.ServerMethods.ConfigUpdated,
            entry =>
            {
                _logger.LogDebug("Config updated: {Key}", entry.Key);
                _cache[entry.Key] = entry;
                SaveCacheToDisk();
                ConfigChanged?.Invoke(this, entry);
            }));

        _subscriptions.Add(hub.On(
            HubConstants.ServerMethods.ConfigReload,
            async () =>
            {
                _logger.LogDebug("Config reload requested");
                await FetchAndCacheAllAsync(CancellationToken.None).ConfigureAwait(false);
            }));
    }
    ...
    private void LoadCacheFromDisk()
    {
        ...
        try
        {
            ...
            _logger.LogDebug("Loaded {Count} config entries from cache", _cache.Count);
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Failed to load config cache from {Path}", _cachePath);
        }
    }
    ...
    private void SaveCacheToDisk()
    {
        ...
        {
            _logger.LogWarning(ex, "Failed to save config cache to {Path}", _cachePath);
        }
    }
