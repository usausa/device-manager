...
internal sealed class MessageClient : IMessageClient, IDisposable
{
    ...
    public async Task SendAsync(string messageType, string content, CancellationToken cancellationToken = default)
    {
        if (_grpcConnectionManager is not null)
        {
            ...
            _logger.LogDebug("Sent message via gRPC: {MessageType}", messageType);
            return;
        }
        ...
        await hub.InvokeAsync(
            HubConstants.ClientMethods.SendMessage,
            messageType,
            content,
            cancellationToken).ConfigureAwait(false);

        _logger.LogDebug("Sent message via SignalR: {MessageType}", messageType);
    }
    ...
    {
        _logger.LogDebug("Received message via gRPC: {MessageType}", msg.Type);
    ...
    private void RegisterHubHandlers()
    {
        ...
        _subscriptions.Add(hub.On<string, string>(
            HubConstants.ServerMethods.ReceiveMessage,
            (messageType, content) =>
            {
                _logger.LogDebug("Received message via SignalR: {MessageType}", messageType);
                MessageReceived?.Invoke(this, (messageType, content));
            }));
    }
